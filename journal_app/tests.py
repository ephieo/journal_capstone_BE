...
class PostsModelTest(TestCase):
    ...
    def testPostExists(self):
        post = Posts.objects.create(
            title='day at park', content='I went to the park')
...
class AllRoutesTest(TestCase):
    ...
    def testPostsPostRequest(self):
        response = self.client.post(
            '/quotes/', {'title': 'clueless', 'content': 'As if!'})
        self.assertEqual(response.status_code, 201)

    def testQuotesPostRequest(self):
        response = self.client.post(
            '/quotes/', {'title': 'trip to athens', 'content': 'I saw the acropolis !!!'})
        self.assertEqual(response.status_code, 201)
